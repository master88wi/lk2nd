...
void lk2nd_fastboot_menu(void)
{
	...
	scale_factor += 1;
	incr = FONT_HEIGHT * scale_factor;
	fbcon_printf_ln(WHITE, y, incr, true, xstr(BOARD));

	scale_factor = old_scale;
	incr = FONT_HEIGHT * scale_factor;

	fbcon_printf_ln(SILVER, y, incr, true, LK2ND_VERSION);
	fbcon_printf_ln(SILVER, y, incr, true, lk2nd_dev.model);
	y += incr;

	fbcon_printf_ln(RED, y, incr, true, "Fastboot mode");
	y += incr;
	...
	fbcon_printf_ln(SILVER, y, incr, true, "Volume keys to navigate.");
	fbcon_printf_ln(SILVER, y, incr, true, "Power key to select.");
	...
	fbcon_printf_ln(WHITE, y, incr, true, "About this device");
